...
export const BRAND_LIST = [
  "Nike", "Adidas", "The North Face", "Carhartt", "Ralph Lauren",
  "Tommy Hilfiger", "Lacoste", "Levi's", "Champion", "Jordan",
];

export const BRAND_INFO = {
  "Nike": { emoji: "✓", color: "#f97316", desc: "Sneakers, Tech Fleece, abbigliamento sportivo" },
  "Adidas": { emoji: "≡", color: "#60a5fa", desc: "Samba, Gazelle, Superstar, tute, felpe" },
  "The North Face": { emoji: "▲", color: "#4ade80", desc: "Nuptse, Denali, Gore-Tex, pile, giacche" },
  "Carhartt": { emoji: "■", color: "#fb923c", desc: "Detroit, workwear, giacche, beanie" },
  "Ralph Lauren": { emoji: "♞", color: "#818cf8", desc: "Polo, Polo Bear, camicie, maglioni" },
  "Tommy Hilfiger": { emoji: "⚑", color: "#f87171", desc: "Flag logo, windbreaker, colorblock" },
  "Lacoste": { emoji: "🐊", color: "#34d399", desc: "Polo L.12.12, felpe, maglioni" },
  "Levi's": { emoji: "✦", color: "#fbbf24", desc: "501, Trucker jacket, jeans vintage" },
  "Champion": { emoji: "C", color: "#c084fc", desc: "Reverse Weave, felpe script logo" },
  "Jordan": { emoji: "J", color: "#f43f5e", desc: "AJ1-AJ13, sneakers da collezione" },
};
...
export function getModelsForBrand(brandName) {
  ...
  const types = Object.keys(db).filter(k => !k.startsWith("_")).map(tipo => ({
    id: tipo, name: tipo, min: db[tipo].min, max: db[tipo].max, isModel: false,
  }));

  /* Modelli specifici */
  const models = db._models ? Object.entries(db._models).map(([key, val]) => ({
    id: key, name: key.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" "),
...
const PRICE_DB = {
  ...
  "Adidas": {
    _tier: "streetwear", _demand: 5, _fakeRisk: "medio", _conf: 80,
    "T-shirt": { min: 6, max: 14 }, "Polo": { min: 10, max: 18 },
    "Felpa": { min: 14, max: 28 }, "Felpa con cappuccio": { min: 16, max: 35 },
    "Giacca": { min: 18, max: 42 }, "Giacca puffer": { min: 28, max: 55 },
    "Giacca a vento": { min: 16, max: 35 }, "Pantaloni": { min: 10, max: 22 },
    "Tuta/Tracksuit": { min: 22, max: 50 }, "Shorts": { min: 6, max: 14 },
    "Sneakers": { min: 18, max: 48 }, "Zaino": { min: 10, max: 28 },
    "Cappello/Berretto": { min: 7, max: 15 }, "Canotta/Top": { min: 5, max: 10 },
    _default: { min: 8, max: 20 },
    _models: {
      "samba": { min: 35, max: 65, conf: 88, note: "Samba OG — top seller 2024-2026, trend fortissimo. Domanda altissima." },
      "samba og": { min: 38, max: 70, conf: 88, note: "Samba OG — il modello più venduto Adidas degli ultimi 2 anni." },
      "gazelle": { min: 28, max: 50, conf: 85, note: "Gazelle — classico ritornato forte. Indoor e normali." },
      "gazelle indoor": { min: 30, max: 55, conf: 85, note: "Gazelle Indoor — trend altissimo 2025-2026." },
      "superstar": { min: 20, max: 40, conf: 88, note: "Superstar — icona shell toe. Sempre costante." },
      "stan smith": { min: 18, max: 35, conf: 88, note: "Stan Smith — classico bianco/verde. Mercato stabile." },
      "campus": { min: 22, max: 42, conf: 83, note: "Campus — retro suede, tornate in trend." },
      "forum": { min: 22, max: 45, conf: 80, note: "Forum Low/Mid — stile basket retro." },
      "ultraboost": { min: 25, max: 55, conf: 82, note: "Ultraboost — running premium, comfort top." },
      "nmd": { min: 18, max: 38, conf: 82, note: "NMD — hype calato rispetto al picco, ancora vendibili." },
      "yeezy": { min: 60, max: 200, conf: 55, note: "Yeezy — prezzo crolla post-separazione Kanye. Fortissimo rischio falsi." },
      "spezial": { min: 30, max: 55, conf: 80, note: "Spezial — nicchia terrace/casual, in crescita." },
      "handball spezial": { min: 32, max: 58, conf: 82, note: "Handball Spezial — trend terrace culture 2025." },
      "sl 72": { min: 22, max: 40, conf: 80, note: "SL 72 — retro runner, trend in crescita." },
      "country": { min: 20, max: 38, conf: 78, note: "Country OG — vintage runner." },
      "ozweego": { min: 18, max: 35, conf: 80, note: "Ozweego — chunky runner, mercato stabile." },
      "zx": { min: 18, max: 38, conf: 78, note: "Serie ZX — retro running." },
      "continental": { min: 15, max: 30, conf: 82, note: "Continental 80 — stile tennis retro." },
      "firebird": { min: 18, max: 35, conf: 82, note: "Firebird — giacca/track jacket iconica con 3 strisce." },
      "beckenbauer": { min: 18, max: 35, conf: 80, note: "Beckenbauer — track jacket classica." },
      "trefoil": { min: 14, max: 30, conf: 80, note: "Logo Trefoil vintage — molto ricercato." },
      "vintage trefoil": { min: 20, max: 45, conf: 78, note: "Vintage con logo Trefoil grande — forte domanda Gen Z." },
      "vintage": { min: 16, max: 40, conf: 72, note: "Generico vintage Adidas. Dipende dal pezzo." },
    },
  },
  "The North Face": {
    _tier: "outdoor", _demand: 5, _fakeRisk: "medio", _conf: 78,
    "Giacca puffer": { min: 38, max: 85 }, "Giacca": { min: 30, max: 70 },
    "Giacca a vento": { min: 22, max: 50 }, "Pile/Fleece": { min: 22, max: 50 },
    "Felpa": { min: 18, max: 38 }, "Felpa con cappuccio": { min: 20, max: 42 },
    "T-shirt": { min: 8, max: 18 }, "Gilet/Smanicato": { min: 25, max: 55 },
    "Zaino": { min: 18, max: 42 }, "Cappello/Berretto": { min: 8, max: 20 },
    _default: { min: 12, max: 30 },
    _models: {
      "nuptse": { min: 65, max: 150, conf: 85, note: "Nuptse — IL puffer TNF. 700 fill il più ricercato. Vendita rapidissima in autunno/inverno." },
      "nuptse 700": { min: 70, max: 160, conf: 85, note: "Nuptse 700 — il modello più venduto TNF in assoluto." },
      "nuptse 1996": { min: 80, max: 180, conf: 82, note: "Nuptse 1996 Retro — prezzo premium per il modello retro." },
      "baltoro": { min: 70, max: 140, conf: 78, note: "Baltoro — puffer premium, meno comune del Nuptse." },
      "himalayan": { min: 60, max: 130, conf: 78, note: "Himalayan parka/puffer — alta gamma TNF." },
      "denali": { min: 35, max: 70, conf: 82, note: "Denali fleece — iconico, sempre richiesto." },
      "gore-tex": { min: 40, max: 90, conf: 78, note: "Gore-Tex — qualsiasi giacca GTX ha un premium." },
      "gore tex": { min: 40, max: 90, conf: 78, note: "Gore-Tex TNF — premium alto." },
      "mountain jacket": { min: 40, max: 80, conf: 80, note: "Mountain Jacket — classico outdoor." },
      "summit series": { min: 45, max: 100, conf: 75, note: "Summit Series — linea top di gamma TNF." },
      "hydrenaline": { min: 25, max: 50, conf: 78, note: "Hydrenaline — giacca a vento leggera." },
      "windwall": { min: 22, max: 45, conf: 80, note: "Windwall — antivento, prezzo medio." },
      "puffer": { min: 40, max: 85, conf: 80, note: "Puffer generico TNF — sempre buona domanda." },
      "fleece": { min: 22, max: 50, conf: 82, note: "Pile/fleece TNF — stabile tutto l'anno." },
      "100 glacier": { min: 18, max: 35, conf: 82, note: "100 Glacier — pile base, prezzo contenuto." },
    },
  },
  "Carhartt": {
    _tier: "streetwear", _demand: 4, _fakeRisk: "basso", _conf: 80,
    "Giacca": { min: 28, max: 60 }, "Felpa": { min: 18, max: 38 },
    "Felpa con cappuccio": { min: 20, max: 42 }, "T-shirt": { min: 8, max: 18 },
    "Pantaloni": { min: 16, max: 32 }, "Cappello/Berretto": { min: 10, max: 22 },
    "Camicia": { min: 15, max: 28 }, "Gilet/Smanicato": { min: 20, max: 40 },
    _default: { min: 12, max: 28 },
    _models: {
      "detroit jacket": { min: 40, max: 85, conf: 85, note: "Detroit Jacket — IL pezzo Carhartt. Icona workwear." },
      "detroit": { min: 40, max: 85, conf: 85, note: "Detroit — icona workwear, vendita rapida." },
      "chore coat": { min: 30, max: 60, conf: 82, note: "Chore Coat — giacca da lavoro classica." },
      "active jacket": { min: 30, max: 65, conf: 82, note: "Active Jacket — hooded, molto richiesta." },
      "michigan coat": { min: 30, max: 55, conf: 80, note: "Michigan — chore coat leggera." },
      "og detroit": { min: 50, max: 100, conf: 78, note: "OG Detroit vintage — prezzo premium." },
      "wip": { min: 22, max: 50, conf: 78, note: "Carhartt WIP (Work In Progress) — linea fashion, prezzi più alti del main." },
      "double knee": { min: 22, max: 45, conf: 82, note: "Double Knee pants — classico workwear." },
      "carpenter": { min: 18, max: 35, conf: 80, note: "Carpenter pants — stile Y2K in trend." },
      "beanie": { min: 10, max: 22, conf: 88, note: "Beanie Carhartt — vendita costante, margini buoni sul costo." },
      "vintage": { min: 25, max: 60, conf: 70, note: "Vintage Carhartt — dipende dallo stato, i pezzi worn-in hanno un seguito." },
    },
  },
  "Ralph Lauren": {
    _tier: "classic", _demand: 4, _fakeRisk: "medio", _conf: 82,
    "Polo": { min: 10, max: 25 }, "Camicia": { min: 10, max: 22 },
    "Felpa": { min: 16, max: 32 }, "Felpa con cappuccio": { min: 18, max: 35 },
    "Maglione": { min: 12, max: 28 }, "Giacca": { min: 22, max: 50 },
    "Cappotto": { min: 28, max: 60 }, "Pantaloni": { min: 10, max: 20 },
    "Cappello/Berretto": { min: 8, max: 20 }, "Cintura": { min: 8, max: 22 },
    "Sciarpa/Foulard": { min: 10, max: 25 },
    _default: { min: 8, max: 22 },
    _models: {
      "big pony": { min: 15, max: 32, conf: 85, note: "Polo Big Pony — logo grande, più appeal su Vinted." },
      "bear": { min: 20, max: 50, conf: 80, note: "Polo Bear — maglioni/felpe con l'orsetto. Molto ricercati." },
      "polo bear": { min: 22, max: 55, conf: 80, note: "Polo Bear — l'orsetto è cult, soprattutto vintage." },
      "sport": { min: 14, max: 30, conf: 78, note: "Polo Sport — linea vintage anni 90, in trend." },
      "polo sport": { min: 16, max: 35, conf: 78, note: "Polo Sport vintage — ricercata dai collector." },
      "flag": { min: 14, max: 28, conf: 80, note: "Con bandiera USA — classico RL." },
      "cable knit": { min: 15, max: 32, conf: 82, note: "Maglione cable knit — classico intramontabile." },
      "oxford": { min: 12, max: 25, conf: 85, note: "Camicia Oxford — sempre richiesta." },
      "harrington": { min: 25, max: 50, conf: 80, note: "Harrington jacket RL — prezzo buono." },
      "vintage": { min: 15, max: 40, conf: 72, note: "Vintage RL — dipende dal pezzo, i 90s valgono di più." },
    },
  },
  "Tommy Hilfiger": {
    _tier: "classic", _demand: 4, _fakeRisk: "basso", _conf: 82,
    "Polo": { min: 8, max: 20 }, "Camicia": { min: 8, max: 20 },
    "Felpa": { min: 14, max: 28 }, "Felpa con cappuccio": { min: 16, max: 32 },
    "Giacca": { min: 20, max: 45 }, "Giacca puffer": { min: 25, max: 50 },
    "Maglione": { min: 10, max: 22 }, "T-shirt": { min: 6, max: 14 },
    _default: { min: 8, max: 20 },
    _models: {
      "flag logo": { min: 14, max: 30, conf: 85, note: "Logo bandiera grande — più appeal, vendita veloce." },
      "big flag": { min: 16, max: 35, conf: 85, note: "Big Flag logo — il più ricercato su Vinted." },
      "colorblock": { min: 16, max: 35, conf: 82, note: "Colorblock — stile anni 90, molto Y2K." },
      "sailing jacket": { min: 25, max: 55, conf: 78, note: "Sailing/Windbreaker — pezzo icona Tommy." },
      "windbreaker": { min: 22, max: 45, conf: 80, note: "Windbreaker Tommy — buona domanda." },
      "jeans": { min: 12, max: 25, conf: 82, note: "Tommy Jeans/Hilfiger Denim — linea giovane." },
      "vintage": { min: 18, max: 45, conf: 72, note: "Vintage Tommy — i pezzi 90s con logo grande valgono di più." },
    },
  },
  "Lacoste": {
    _tier: "classic", _demand: 4, _fakeRisk: "basso", _conf: 82,
    "Polo": { min: 10, max: 25 }, "Felpa": { min: 14, max: 28 },
    "Maglione": { min: 12, max: 25 }, "Giacca": { min: 18, max: 40 },
    "T-shirt": { min: 6, max: 14 }, "Cappello/Berretto": { min: 8, max: 18 },
    "Camicia": { min: 10, max: 20 },
    _default: { min: 8, max: 20 },
    _models: {
      "l.12.12": { min: 12, max: 28, conf: 88, note: "L.12.12 — LA polo Lacoste originale. Best seller eterno." },
      "classic fit": { min: 10, max: 25, conf: 85, note: "Polo classic fit — la più venduta." },
      "slim fit": { min: 10, max: 25, conf: 85, note: "Polo slim fit — vestibilità moderna, buona domanda." },
      "chemise": { min: 14, max: 30, conf: 78, note: "Chemise Lacoste — etichetta vintage, vale di più." },
      "vintage": { min: 14, max: 32, conf: 72, note: "Vintage Lacoste — l'etichetta 'Chemise Lacoste' alza il prezzo." },
      "sport": { min: 10, max: 22, conf: 80, note: "Lacoste Sport — linea tecnica, prezzi medi." },
      "live": { min: 12, max: 25, conf: 78, note: "Lacoste Live — linea giovane, più streetwear." },
    },
  },
  "Levi's": {
    _tier: "classic", _demand: 4, _fakeRisk: "basso", _conf: 85,
    "Jeans": { min: 12, max: 30 }, "Giacca": { min: 22, max: 50 },
    "T-shirt": { min: 6, max: 14 }, "Camicia": { min: 10, max: 22 },
    "Shorts": { min: 8, max: 18 },
    _default: { min: 8, max: 20 },
    _models: {
      "501": { min: 18, max: 40, conf: 90, note: "501 — IL jeans. Originale, sempre richiesto. Vintage vale di più." },
      "501 original": { min: 20, max: 42, conf: 90, note: "501 Original Fit — classico intramontabile." },
      "501 vintage": { min: 25, max: 55, conf: 82, note: "501 vintage — Made in USA vale premium." },
      "505": { min: 14, max: 28, conf: 85, note: "505 Regular — secondo più venduto Levi's." },
      "511": { min: 12, max: 25, conf: 85, note: "511 Slim — fit moderno, buona domanda." },
      "512": { min: 12, max: 25, conf: 85, note: "512 Slim Taper — fit contemporaneo." },
      "517": { min: 15, max: 32, conf: 80, note: "517 Bootcut — trend vintage in risalita." },
      "550": { min: 14, max: 30, conf: 82, note: "550 Relaxed — baggy fit in trend Y2K." },
      "trucker jacket": { min: 28, max: 60, conf: 85, note: "Trucker Jacket — giacca di jeans iconica. Sherpa lined vale di più." },
      "trucker": { min: 28, max: 60, conf: 85, note: "Trucker — classico Levi's, sempre vendibile." },
      "sherpa": { min: 32, max: 65, conf: 83, note: "Sherpa Trucker — con pelliccia interna, premium invernale." },
      "ex boyfriend": { min: 15, max: 28, conf: 82, note: "Ex-Boyfriend — fit oversize donna, trend." },
      "ribcage": { min: 16, max: 30, conf: 82, note: "Ribcage — vita alta donna, molto richiesto." },
      "vintage": { min: 18, max: 50, conf: 70, note: "Vintage Levi's — Made in USA e arancione tab valgono premium." },
    },
  },
  "Champion": {
    _tier: "streetwear", _demand: 4, _fakeRisk: "basso", _conf: 82,
    "Felpa": { min: 12, max: 28 }, "Felpa con cappuccio": { min: 15, max: 32 },
    "T-shirt": { min: 6, max: 14 }, "Tuta/Tracksuit": { min: 18, max: 38 },
    "Pantaloni": { min: 10, max: 20 }, "Giacca a vento": { min: 15, max: 30 },
    _default: { min: 7, max: 18 },
    _models: {
      "reverse weave": { min: 22, max: 48, conf: 85, note: "Reverse Weave — la linea premium Champion. Vale il doppio della basic." },
      "script logo": { min: 14, max: 30, conf: 82, note: "Logo script (corsivo grande) — più appeal della C piccola." },
      "big logo": { min: 14, max: 30, conf: 82, note: "Logo grande — si vende più veloce." },
      "vintage": { min: 16, max: 40, conf: 72, note: "Vintage Champion — Made in USA vale molto di più." },
      "made in usa": { min: 25, max: 55, conf: 78, note: "Made in USA — premium alto, collector item." },
    },
  },
  "Jordan": {
    _tier: "streetwear", _demand: 5, _fakeRisk: "alto", _conf: 55,
    "Sneakers": { min: 35, max: 120 }, "Felpa": { min: 18, max: 38 },
    "Felpa con cappuccio": { min: 20, max: 42 },
    "T-shirt": { min: 10, max: 22 }, "Giacca": { min: 28, max: 55 },
    "Pantaloni": { min: 14, max: 28 }, "Shorts": { min: 8, max: 18 },
    _default: { min: 12, max: 35 },
    _models: {
      "jordan 1": { min: 50, max: 130, conf: 72, note: "AJ1 — icona assoluta. Prezzo varia enormemente per colorway. FALSI OVUNQUE." },
      "jordan 1 high": { min: 55, max: 150, conf: 70, note: "AJ1 High OG — le più ricercate. Chicago, Bred, Royal = top price." },
      "jordan 1 mid": { min: 35, max: 70, conf: 78, note: "AJ1 Mid — meno hype della High ma più accessibile. Buon volume." },
      "jordan 1 low": { min: 35, max: 65, conf: 78, note: "AJ1 Low — cresciute molto in popolarità." },
      "jordan 3": { min: 50, max: 120, conf: 70, note: "AJ3 — White Cement è iconica. Attenzione falsi." },
      "jordan 4": { min: 55, max: 150, conf: 68, note: "AJ4 — tra le più ricercate. White Cement, Bred, Military Black. Falsi comunissimi." },
      "jordan 4 retro": { min: 60, max: 160, conf: 68, note: "AJ4 Retro — best seller su StockX 2025." },
      "jordan 5": { min: 45, max: 100, conf: 72, note: "AJ5 — classica, buona domanda." },
      "jordan 6": { min: 40, max: 95, conf: 72, note: "AJ6 — Infrared è la più ricercata." },
      "jordan 11": { min: 55, max: 140, conf: 70, note: "AJ11 — Bred, Concord, Space Jam. Top seller storica." },
      "jordan 12": { min: 45, max: 100, conf: 72, note: "AJ12 — classica, Flu Game è cult." },
      "jordan 13": { min: 40, max: 90, conf: 72, note: "AJ13 — He Got Game, Chicago." },
    },
  },
};
